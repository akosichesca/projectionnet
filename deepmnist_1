...
def random_projections(T,d,x):
  RandomPlanes = tf.Variable(tf.random_normal([1,d,T], mean = 0, stddev = 100), name = "Planes", trainable=True)
  RandomPlanes2 = tf.tile(RandomPlanes,[FLAGS.BATCHSIZE,1,1])
...
def main(_):
  ...
  with tf.name_scope('LSH'):
    ...
    xtile = tf.tile(x,[1,T])
    ProjectedValues = tf.matmul(RandomPlanes, tf.reshape(xtile,[-1,T,784]))
    ProjectedFlat = tf.reshape(ProjectedValues,[-1,d*784])

    W_fcP = weight_variable([d*784, 10]) #size of figure, numclasses
  ...
  entropy = tf.losses.cosine_distance(y_P, y_conv,1)
  print(entropy)
  ...
  with tf.name_scope('loss'):
    ...
    cross_entropy = tf.reduce_mean(cross_entropy) #+ P_loss + 0.01*tf.log(E_loss)

  W_loss = weight_variable([3,1])
  loss = tf.reshape([cross_entropy, P_loss, tf.log(E_loss)], [1,3])
  ...
  with tf.Session() as sess:
    ...
    for i in range(FLAGS.TEST_ITER):
        batch_test = mnist.test.next_batch(FLAGS.BATCHSIZE)
        acc = acc + accuracy_P.eval(feed_dict={
              x:batch_test[0] , y_: batch_test[1], keep_prob: 1.0})
